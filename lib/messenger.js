...
class Messenger {
  constructor(name, config) {
    ...
    // group.
    const { kafka, ...others} = JSON.parse(JSON.stringify(this.config));
    ...
    this._tenancyConsumer = new Consumer({ kafka }, "tenancy consumer");

    this.consumer = new Consumer(this.config, "normal consumer");
  }
  ...
  _initTenancyConsumer(resolve, reject, counter = undefined) {
    ...
    this.topicManager.getTopic(subject, tenant, this.config.databroker.url, true).then((topic) => {
      ...
      if (counter > 0) {
        ...
        }, this.config.databroker.timeoutSleep);
      } else {
